...
module DigitalCommonsImportInformation
  extend KObjectURLs

  APPLICATION_ID = 45324533

  OUTPUT_FILE = "#{File.dirname(KFRAMEWORK_LOG_FILE)}/app#{APPLICATION_ID}-digital-commons-mapping-info.json"
  ...
  def self.run2
    ...
    KApp.with_pg_database do |db|
      sql = "SELECT _object, digitalcommonsurl FROM #{KApp.db_schema_name}.#{plugin_db_table_name('hres_repo_digital_commons', 'legacyurlsforobjects')} ORDER BY _object"
      results = db.exec(sql)
      puts "  count: #{results.length}"
      results.each do |ref_s,dc_url|
    ...
    puts "Digital Commons mapping information saved to #{OUTPUT_FILE}"
    puts "Download this file and use with the digital-commons-generate-redirects-and-mappings.rb script."
  end
...
DigitalCommonsImportInformation.run
